...
class UserStory09(UserStory):
    ...
    def get_rows(self, conn):
        ...
        for row in rows:
            birthday = c.execute('SELECT Birthday FROM INDI WHERE ID = "{}"'.format(row[1])).fetchone()
            death = c.execute('SELECT Death FROM INDI WHERE ID = "{}"'.format(row[2])).fetchone()
            ...
            if(birthday and death):
                ...
                if(datetime.strptime(birthday[0], '%Y-%m-%d') > (datetime.strptime(death[0], '%Y-%m-%d') + timedelta(days=270))):
                    res.append(row + ('Husband', )) #TODO: Must be a better way to do this
