...
class UserStory13(UserStory):
    ...
    def print_rows(self, rows):
        for row in rows:
            print('CURIOSITY: Family {0} has strange sibling spacing between children {1} and {2} born on {3}, and {4}'.format(row))
    ...
    def get_rows(self, conn):
        ...
        for family in families:
            children = c.execute('SELECT INDI_ID, Birthday FROM CHLD JOIN INDI ON (INDI.ID = INDI_ID) WHERE Birthday != "NA"').fetchall() or []
            if(len(children) > 1):
